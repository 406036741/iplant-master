package com.iplant.presenter.view.Layout;
...
public class DragLayout extends FrameLayout {
    ...
    private float mFirstTouchDownX = 0;//拖拽时,起点的x轴坐标
    ...
    private ViewDragHelper.Callback dragHelperCallback = new ViewDragHelper.Callback() {
        ...
        public boolean tryCaptureView(View child, int pointerId) {
            boolean wIsCapture = true;
            if (Math.abs(mFirstTouchDownX) >width*0.1)
                wIsCapture = false;
            mFirstTouchDownX = 0;
            return wIsCapture;
        }
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            ...
            if (xvel > 0) {
                ...
                close();
            } else if (releasedChild == vg_main && mainLeft > range * 0.7) {
                open();
    ...
    protected void onFinishInflate() {
        ...
        if (isShowShadow) {
            ...
            iv_shadow.setImageResource(R.drawable.darglayoutshadow);
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        if (mFirstTouchDownX == 0)
            mFirstTouchDownX = ev.getX();
